...
export default function CartPage() {
  ...
  // Add item to cart (Ensures no duplicate entries)
  const addToCart = (newItem: CartItem) => {
    const storedCart: CartItem[] = JSON.parse(localStorage.getItem("cart") || "[]");

    const existingItemIndex = storedCart.findIndex((item) => item._id === newItem._id);

    if (existingItemIndex !== -1) {
      // Item already exists, update quantity
      storedCart[existingItemIndex].quantity = Math.min(
        storedCart[existingItemIndex].quantity + newItem.quantity,
        storedCart[existingItemIndex].stock
      );
    } else {
